...
npm install -g @magdox/cli            # any platform with Node 18+
...
Every checksum file is signed with Sigstore cosign.
The installers verify SHA-256 checksums obtained over HTTPS; they do not
currently verify the cosign signature. The shell installer requires Python 3.9+
for bounded archive validation and defaults to `$HOME/.local/bin`. Set
`MAGDOX_INSTALL_DIR` to another directory you own. Install and run as your
ordinary user: the CLI and installers do not request elevation or scan as root.
